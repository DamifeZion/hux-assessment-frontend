...
export const Navbar: React.FC<TNavbarProps> = ({ className }) => {
   ...
   return (
      <nav className={cn("bg-background/80 backdrop-blur-xl", className)}>
                  ...
                  <Sheet open={openSheet} onOpenChange={() => setOpenSheet(prev => !prev)}>
                     ...
                     <SheetContent side="left" className="py-10">
                        <SheetHeader className="mb-6 *:sr-only [&_:first-child]:not-sr-only">
                           <SheetTitle>
                              <Logo className="!px-4" />
                              <Typography variant="p" affects="small" className="sr-only">
                                 Mobile menu
                              </Typography>
                           </SheetTitle>
                           <SheetDescription>
                              This menu includes the {navbarConst.ROUTES.map((route) => (
                                 <span key={route.url}>
                                    {route.title.toLowerCase()}{", "}
                                 </span>
                              ))}
                              and also the theme toggle and action button to get started
                           </SheetDescription>
                        </SheetHeader>
